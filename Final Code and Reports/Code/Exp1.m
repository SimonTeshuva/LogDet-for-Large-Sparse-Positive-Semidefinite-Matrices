...
% experiment 1a compares the runtime of the three methods. the
% function runtime_comparison(rs,order,samp,kappa) produces the data, and
% the results are plotted here on a semilogx plot. 

% rs = random vector size, order = order of function used, samp = number of
% samples used, kappa = condition number of kneser matrices
if ex1_do(1)
    fprintf('running 1a on sizes;');fprintf('%.f,',rs);fprintf('\n');
    [crt,rrt,ckt,rkt,chol_rt,chol_kt]=runtime_comparison(rs,order,samp,kappa);
    % crt = chebyshev function on random matrix time
    % rrt = rational function on random matrix time
    % ckt = chebyshev function on kneser matrix time
    % rrt = rational function on kneser  matrix time
    % chol_rt = cholesky decomposition on random matrix time
    % chol_rt = cholesky decomposition on kneser matrix time
    k_vect=1:length(rs);
    n_vect=2*k_vect+1;
...
% experiment 1b compares the relative accuracy of the three methods on
% mediym sized matrices. The function 
% relative_accuracy_medium(rsra_chol,order,samp,kappa) 
% produces the data, and the results are plotted here on a semilogx plot. 

% rs = random vector size for use on relative accuracy of medium matrix size,
% order = order of function used, samp = number of samples used, 
if ex1_do(2)
    ...
    % kappa = condition number of kneser matrices
    % rarr = relative accuracy on random matrix using rational algorithm
    % rarc = relative accuracy on random matrix using cholesky algorithm
    % rakr = relative accuracy on kneser matrix using rational algorithm
    % rakc = relative accuracy on kneser matrix using chebyshev algorithm
    k_vect=1:length(rs);
    n_vect=2*k_vect+1;
    for i=1:length(k_vect)
        ks(i)=nchoosek(n_vect(i),k_vect(i));
...
% experiment 1c compares the relative accuracy of the chebyshev and 
% rational methods on large matrices. the function 
% relative_accuracy_large(k_vect,order,samp,kappa)
% produces the data, and the results are plotted here on a semilogx plot. 

% k_vect = the values of k for (n,k) kneser graphs to be generated
% order = order of function used, samp = number of samples used, 
% kappa = condition number of kneser matrices
if ex1_do(3)
    k_vect=5:11;
    ...
    % rake_rat = relative accuracy from kneser matrices exact logdet using
    % rational fuction method
    % rake_cheb = relative accuracy from kneser matrices exact logdet using
    % chebyshev fuction method
    n_vect=2*k_vect+1;
    for i=1:length(k_vect)
        ks(i)=nchoosek(n_vect(i),k_vect(i));
    end
...
% order = order of function used, samp = number of samples used
% kappa = condition number of kneser matrices
% pertubations = a set of numbers that will be multiplied by either lmin
% (if <1) or lmax (if >1). introduces uncertainty and error in Chebysev
% Algorithm
if ex1_do(4)
	fprintf('running 1a on pertubations;');fprintf('%.f,',pertubations);fprintf('\n');
    k=11; % use the (23, 11) kneser graph for this experiment.
...
% for each matrix size given, evaluate the logdet using the three algorithms
% then store the time taken in a vector. each algorithm has been
% restricted to a maximum size, as beyond that size the time taken to
% compute gets very long. 

% when switching between random matrices and kneser matrices, the old
% matrix, and some other data are cleared to save memory.
function [crt,rrt,ckt,rkt, chol_rt,chol_kt] = runtime_comparison(rs,order,samp,kappa)
diag_dom_const=1e-3;
ks=1:length(rs);
for i=1:max(ks)    clear lmin lmax A Asize;
    Asize=rs(i);
...
% of kappa, the exact logdet is easily computed and is used to determine the
% relative accuracy. 
...
function [logdet,lmin,lmax] = ld_kneser(n,k,kappa)
    ...
    mults = [];
    for i = 0:k
        eval = ((-1)^i)*nchoosek(n-k-i, k-i);  % formula for the eigenvalues of the kneser gaph
        
        if i == 0 % formula for the multiplicities of the eigenvalues of the kneser gaph
            mult = 1;
        else 
	...
	epsilon = (max(evals)-min(evals))/(kappa - 1); % number to be added to the 
    % diagonal entries of the matrix to set the condition number to kappa
    evals = evals - min(evals) + epsilon;
    
    logdet = sum(mults.*log(evals)); % analytical formula for the logdet of a matrix
    lmin=min(evals);
    lmax=max(evals);
end